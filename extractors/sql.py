...
@sql_bp.route('/extract_sql_update', methods=['POST'])
def extract_sql_update():
    if 'file' not in request.files:
        return jsonify({"error": "No file part"}), 400
    file = request.files['file']
    if file.filename == '':
        ...
        try:
            with zipfile.ZipFile(file, 'r') as zip_ref:
                ...
                for filename in zip_ref.namelist():
                    if filename.endswith('.cbl'):
                        with zip_ref.open(filename) as cobol_file:
                            sql_queries = process_cobol_file_for_update(cobol_file)
                            if sql_queries:
                                results[filename] = sql_queries
                if results:
                    return jsonify(results), 200
...
@sql_bp.route('/extract_sql_delete', methods=['POST'])
def extract_sql_delete():
    if 'file' not in request.files:
        return jsonify({"error": "No file part"}), 400
    file = request.files['file']
    if file.filename == '':
        ...
        try:
            with zipfile.ZipFile(file, 'r') as zip_ref:
                ...
                for filename in zip_ref.namelist():
                    if filename.endswith('.cbl'):
                        with zip_ref.open(filename) as cobol_file:
                            sql_queries = process_cobol_file_for_delete(cobol_file)
                            if sql_queries:
                                results[filename] = sql_queries
                if results:
                    return jsonify(results), 200
